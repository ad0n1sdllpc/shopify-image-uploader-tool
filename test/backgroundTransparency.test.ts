...
describe("white background transparency", () => {
  it("turns white and near-white pixels transparent while preserving tile colors", async () => {
    const directory = await fs.mkdtemp(path.join(os.tmpdir(), "tile-transparency-"));
    const sourcePath = path.join(directory, "tile.jpg");
    ...
    expect(upload.fileName).toBe("tile.png");
