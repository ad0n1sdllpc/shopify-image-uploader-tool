...
import { buildProductMigrationScan, extractMigrationMetafields, isIgnorableInventoryActivationError, metafieldInputs } from "@/lib/productMigration";

type MigrationProductNode = Parameters<typeof buildProductMigrationScan>[0][number];

function product(prefix: "LUZ" | "VIS" | "MIN", overrides: Partial<MigrationProductNode> = {}): MigrationProductNode {
  const baseSku = "YM6623";
  const sku = `${prefix}-${baseSku}`;
  const locationName = prefix === "LUZ" ? "Lusterplus Inc." : prefix === "VIS" ? "ARTEMISIA CEBU" : "ARTEMISIA DAVAO";

  return {
    id: overrides.id ?? `gid://shopify/Product/${prefix}`,
    title: overrides.title ?? sku,
    handle: overrides.handle ?? sku.toLowerCase(),
    descriptionHtml: overrides.descriptionHtml ?? "<p>Porcelain Inkjet Print. Water absorption E&lt;0.5%. Thickness 8.2mm. Rectified. Traffic rating Moderate.</p>",
      ...
      nodes: [
        {
          ...
          inventoryItem: {
            ...
            inventoryLevels: {
              nodes: [
                {
                  location: { id: `gid://shopify/Location/${prefix}`, name: locationName },
                  quantities: [{ name: "available", quantity: prefix === "LUZ" ? 3 : prefix === "VIS" ? 5 : 7 }]
                }
              ]
            }
          }
    ...
    media: overrides.media ?? {
      nodes: [
        { id: `media-${prefix}-1`, image: { url: "https://cdn/shared.jpg" }, preview: null },
        { id: `media-${prefix}-2`, image: { url: `https://cdn/${prefix}.jpg` }, preview: null }
      ]
    },
...
describe("product migration grouping", () => {
  it("builds a complete regional candidate with mapped inventory and canonical images", () => {
    ...
    expect(scan.candidates[0].regionalProducts.map((item) => [item.prefix, item.locationName, item.quantity])).toEqual([
  ...
  it("marks candidates when a unified product already exists", () => {
    ...
    expect(scan.candidates[0].existingUnifiedProductId).toBe("gid://shopify/Product/unified");
    expect(scan.candidates[0].manualReviewFields).toContain("existing_unified_product");
  });
});
...
describe("product migration metafield extraction", () => {
  ...
  it("serializes list metafields as Shopify JSON array values", () => {
    ...
    expect(inputs.find((input) => input.key === "surface_finish")).toMatchObject({
      type: "list.single_line_text_field",
      value: JSON.stringify(["Polished"])
    });
    expect(inputs.find((input) => input.key === "application_area")).toMatchObject({
      type: "list.single_line_text_field",
      value: JSON.stringify(["Floor", "Indoor"])
    });
    expect(inputs.find((input) => input.key === "region_availability")).toMatchObject({
      type: "list.single_line_text_field",
...
describe("product migration inventory activation", () => {
  it("treats already active inventory activation errors as idempotent success", () => {
    expect(isIgnorableInventoryActivationError({ message: "Inventory item is already stocked at this location.", code: "ITEM_ALREADY_STOCKED" })).toBe(true);
    expect(isIgnorableInventoryActivationError({ message: "Inventory level is already active.", code: null })).toBe(true);
  });

  it("does not ignore unrelated inventory activation errors", () => {
    expect(isIgnorableInventoryActivationError({ message: "Location does not exist.", code: "LOCATION_NOT_FOUND" })).toBe(false);
  });
});
